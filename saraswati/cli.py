...
channels_opt = click.option("--channel", "-c", "channels", type=click.STRING, multiple=True, callback=validate_channel, help="Define channels to record. Add multiple times to define more channels")
spool_opt = click.option("--spool", type=click.Path(exists=True, file_okay=False, dir_okay=True, writable=True), help="Absolute path to the spool directory")
...
@cloup.group("saraswati", context_settings=CONTEXT_SETTINGS, invoke_without_command=True)
@click.option(
    '--version',
...
@channels_opt
@spool_opt
@click.option("--chunk-duration", type=click.INT, help="Duration of each chunk file (seconds)", default=60)
@click.option("--chunk-max-files", type=click.INT, help="Maximum number of file fragments", default=9999)
@debug_opt
def record(channels: List[Channel], chunk_duration: int, chunk_max_files: int, spool: str, debug: bool):
